...
#define DMAIOBASE io_p2v(LPC18XX_DMA_BASE)
#define VALID_CHANNEL(c) (((c) >= 0) && ((c) < MAX_DMA_CHANNELS))

/*
 * Bit fields of the DMA Request Select register
 */
#define LPC18XX_SCC_DMACREQSEL_SEL6_I2S0_MSK	(1 << 6)
#define LPC18XX_SCC_DMACREQSEL_SEL7_I2S1_MSK	(1 << 7)
...
static int lpc18xx_ch_setup(struct dma_config *dmachcfg)
{
	...
	if (dmachcfg->src_ahb1)
		tmpctrl |= DMAC_CHAN_SRC_AHB1;
	if (dmachcfg->dst_ahb1)
		tmpctrl |= DMAC_CHAN_DST_AHB1;
...
int lpc18xx_dma_ch_enable(int ch)
{
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_ch_enable);

int lpc18xx_dma_ch_disable(int ch)
{
	if (!VALID_CHANNEL(ch) || !dma_ctrl.dma_channels[ch].name)
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_ch_disable);

int lpc18xx_dma_ch_get(struct dma_config *dmachcfg, char *name,
	...
	__dma_regs_lock();
	dma_clocks_up();
	ret = lpc18xx_ch_setup(dmachcfg);
	__dma_regs_unlock();

	return ret;
}
EXPORT_SYMBOL_GPL(lpc18xx_dma_ch_get);

int lpc18xx_dma_ch_put(int ch)
{
	u32 tmp;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_ch_put);

int lpc18xx_dma_ch_pause_unpause(int ch, int pause) {
	u32 tmp;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_ch_pause_unpause);

int lpc18xx_dma_start_pflow_xfer(int ch,
				void *src,
				void *dst,
				int enable)
{
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_start_pflow_xfer);

int lpc18xx_dma_is_active(int ch)
{
	int active = 0;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_is_active);

extern u32 lpc18xx_dma_llist_v_to_p(int ch, u32 vlist)
{
	u32 pptr;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_llist_v_to_p);

u32 lpc18xx_dma_llist_p_to_v(int ch, u32 plist)
{
	u32 vptr;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_llist_p_to_v);

u32 lpc18xx_dma_alloc_llist(int ch, int entries)
{
	int i;
	...
	for (i = 0; i < entries; i++) {
		...
		pdmalistst->next_list_phy = lpc18xx_dma_llist_v_to_p(
			ch, (u32) pdmalistst->next_list_addr);
		pdmalistst->prev_list_phy = lpc18xx_dma_llist_v_to_p(
			ch, (u32) pdmalistst->prev_list_addr);
		pdmalistst++;
	}
	pdmalist[entries - 1].next_list_addr = pdmalist;
	pdmalist[entries - 1].next_list_phy = lpc18xx_dma_llist_v_to_p(ch,
		(u32) pdmalist[entries - 1].next_list_addr);
	pdmalist->prev_list_addr = &pdmalist[entries - 1];
	pdmalist->prev_list_phy = lpc18xx_dma_llist_v_to_p(
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_alloc_llist);

void lpc18xx_dma_dealloc_llist(int ch) {
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_dealloc_llist);

extern u32 lpc18xx_dma_get_llist_head(int ch) {
	if ((!VALID_CHANNEL(ch)) || (dma_ctrl.dma_channels[ch].name == NULL) ||
		(dma_ctrl.dma_channels[ch].list_vstart == 0))
		return 0;
	...
	/* Return the current list pointer (virtual) for the
	   DMA channel */
	return lpc18xx_dma_llist_p_to_v(ch,
		__raw_readl(DMACH_LLI(DMAIOBASE, ch)));
}
EXPORT_SYMBOL_GPL(lpc18xx_dma_get_llist_head);

extern void lpc18xx_dma_flush_llist(int ch) {
	if ((!VALID_CHANNEL(ch)) || (dma_ctrl.dma_channels[ch].name == NULL) ||
		(dma_ctrl.dma_channels[ch].list_vstart == 0))
		return;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_flush_llist);

u32 lpc18xx_dma_queue_llist_entry(int ch, void *src, void *dst, int size)
{
	...
	plhead->prev_list_addr->dmall.next_lli =
		lpc18xx_dma_llist_v_to_p(ch, (u32) plhead);
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_queue_llist_entry);

extern u32 lpc18xx_get_free_llist_entry(int ch) {
...
EXPORT_SYMBOL_GPL(lpc18xx_get_free_llist_entry);

int lpc18xx_dma_start_xfer(int ch, u32 config)
{
	struct dma_list_ctrl *plhead;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_start_xfer);

u32 lpc18xx_dma_queue_llist(int ch, void *src, void *dst, int size, u32 ctrl)
{
	struct dma_list_ctrl *plhead;
...
EXPORT_SYMBOL_GPL(lpc18xx_dma_queue_llist);

extern void lpc18xx_dma_force_burst(int ch, int src)
{
	__raw_writel(1 << src, DMA_SW_BURST_REQ(DMAIOBASE));
}
EXPORT_SYMBOL_GPL(lpc18xx_dma_force_burst);
...
void __init lpc18xx_dma_init(void)
{
	...
	ret = request_irq(LPC18XX_DMA_IRQ, dma_irq_handler, 0, "DMA", NULL);
	if (ret) {
	...
	if (IS_ERR(dma_ctrl.clk)) {
		printk(KERN_CRIT "Can't get clock for DMA controller\n");
		ret = -ENODEV;
		goto errout;
	}
	clk_enable(dma_ctrl.clk);
