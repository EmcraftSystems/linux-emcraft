...
/*
 * "Interrupt ID" in Table 43 in the LPC18XX/43XX User Manual (page 90)
 */
#define LPC18XX_LCD_IRQ		7
 ...
 */
static struct clcd_panel ea_lpc18xx_lcd_004_panel = {
	.mode		= {
...
out:
	if (ret < 0) {
		printk(KERN_ERR "lpc18xx fb: Could not configure "
			"the LCD screen on the EA-LCD-004 board: %d\n", ret);
	}
 ...
 */
static struct i2c_board_info __initdata ea_lpc18xx_lcd_004_dimmer = {
...
static int lpc18xx_clcd_setup(struct clcd_fb *fb)
{
	int platform;
	int ret;
	dma_addr_t dma;
	...
	case PLATFORM_LPC18XX_EA_LPC4357_EVAL:
		fb->panel = &ea_lpc18xx_lcd_004_panel;
		break;
	default:
		break;
	}
...
static int lpc18xx_clcd_mmap(struct clcd_fb *fb, struct vm_area_struct *vma)
{
	/*
	 * There is no MMU on LPC18Xx/7x, no need to call
	 * `dma_mmap_writecombine`.
	 */
	return 0;
}

static void lpc18xx_clcd_remove(struct clcd_fb *fb)
{
	dma_free_writecombine(&fb->dev->dev, fb->fb.fix.smem_len,
		fb->fb.screen_base, fb->fb.fix.smem_start);
}

static struct clcd_board lpc18xx_fb_data = {
	.name		= "LPC18xx/43xx LCDC",
	.check		= clcdfb_check,
	.decode		= clcdfb_decode,
	.disable	= NULL,
	.enable		= NULL,
	.setup		= lpc18xx_clcd_setup,
	.mmap		= lpc18xx_clcd_mmap,
	.remove		= lpc18xx_clcd_remove,
};


static struct amba_device lpc18xx_fb_device = {
	.dev = {
		.coherent_dma_mask	= ~0,
		.init_name		= "dev:clcd",
		.platform_data		= &lpc18xx_fb_data,
	},
	.res = {
		.start			= LPC18XX_LCD_BASE,
		.end			= LPC18XX_LCD_BASE + SZ_4K - 1,
		.flags			= IORESOURCE_MEM,
	},
	.dma_mask			= ~0,
	.irq				= { LPC18XX_LCD_IRQ, NO_IRQ },
};
...
void __init lpc18xx_fb_init(void)
{
	...
	case PLATFORM_LPC18XX_EA_LPC4357_EVAL:
		...
		ret = i2c_register_board_info(0, &ea_lpc18xx_lcd_004_dimmer, 1);
		if (ret < 0)
			goto out;
		break;
	default:
	...
	amba_device_register(&lpc18xx_fb_device, &iomem_resource);

out:
	if (ret < 0) {
		printk(KERN_ERR "lpc18xx fb: Could not initialize "
			"the LCD screen.\n");
	}
}
