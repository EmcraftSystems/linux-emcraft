...
static struct s3c_hsotg		*the_controller;
...
static inline struct s3c_hsotg_ep *index_to_ep(struct s3c_hsotg *hsotg,
...
static void s3c_hsotg_init_fifo(struct s3c_hsotg *hsotg)
{
	...
	writel((rx_fifo_sz << FIFOSIZE_STARTADDR_SHIFT) |
	       (tx_fifo_sz << FIFOSIZE_DEPTH_SHIFT),
...
static int s3c_hsotg_handle_unaligned_buf_start(struct s3c_hsotg *hsotg,
...
static void s3c_hsotg_handle_unaligned_buf_complete(struct s3c_hsotg *hsotg,
	struct s3c_hsotg_ep *hs_ep, struct s3c_hsotg_req *hs_req)
{
...
static void s3c_hsotg_program_zlp(struct s3c_hsotg *hsotg,
				  struct s3c_hsotg_ep *hs_ep)
{
	u32 ctrl;
 ...
 * the TxFIFO.
 */
static void s3c_hsotg_ep0_zlp(struct s3c_hsotg *hsotg, bool dir_in)
{
...
static void s3c_hsotg_disconnect(struct s3c_hsotg *hsotg)
{
	...
	for (i = 0; i < hsotg->num_of_eps; i++) {
		if (hsotg->eps_in[i])
			kill_all_requests(hsotg, hsotg->eps_in[i], -ESHUTDOWN);
		if (hsotg->eps_out[i])
			kill_all_requests(hsotg, hsotg->eps_out[i], -ESHUTDOWN);
	}
					     ...
					     bool is_usb_reset)
{
	...
	writel(hsotg->phyif | GUSBCFG_TOUTCAL(7) | val | GUSBCFG_FORCEDEVMODE,
	       hsotg->regs + GUSBCFG);
...
void s3c_hsotg_core_connect(struct s3c_hsotg *hsotg)
{
...
irq_retry:
	...
	if (gintsts & GINTSTS_SESSREQINT) {
		dev_dbg(hsotg->dev, "GINTSTS_SessReq\n");
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	...
	mps = __le16_to_cpu(desc->wMaxPacketSize);
...
static void s3c_hsotg_init(struct s3c_hsotg *hsotg)
{
	...
	writel(hsotg->phyif | GUSBCFG_TOUTCAL(7) | trdtim |
		GUSBCFG_FORCEDEVMODE, hsotg->regs + GUSBCFG);
...
/**
 * dwc2_udc_getframe - read the frame number
 * @gadget: The usb gadget state
 *
 * Read the {micro} frame number
 */
static int dwc2_udc_getframe(struct usb_gadget *gadget)
{
	return s3c_hsotg_read_frameno(to_hsotg(gadget));
}

/**
 * dwc2_udc_pullup - connect/disconnect the USB PHY
 * @gadget: The usb gadget state
 * @is_on: Current state of the USB PHY
 *
 * Connect/Disconnect the USB PHY pullup
 */
static int dwc2_udc_pullup(struct usb_gadget *gadget, int is_on)
{
	struct s3c_hsotg *hsotg = to_hsotg(gadget);
	unsigned long flags = 0;
...
static int dwc2_udc_vbus_session(struct usb_gadget *gadget, int is_active)
{
	struct s3c_hsotg *hsotg = to_hsotg(gadget);
	unsigned long flags;
...
static const struct usb_gadget_ops s3c_hsotg_gadget_ops = {
	.get_frame	= dwc2_udc_getframe,
	.pullup		= dwc2_udc_pullup,
	.vbus_session	= dwc2_udc_vbus_session,
};
...
static void s3c_hsotg_initep(struct s3c_hsotg *hsotg,
			     struct s3c_hsotg_ep *hs_ep, int epnum,
			     bool dir_in)
{
	u32 ptxfifo;
	char *dir;
...
static int s3c_hsotg_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < hsotg->num_of_eps; i++) {
		if (hsotg->eps_in[i])
			s3c_hsotg_initep(hsotg, hsotg->eps_in[i], i, 1);
		if (hsotg->eps_out[i])
			s3c_hsotg_initep(hsotg, hsotg->eps_out[i], i, 0);
	}
	...
	hsotg->gadget.dev.parent = &pdev->dev;
	hsotg->gadget.dev.dma_mask = NULL;
	hsotg->gadget.dev.init_name = dev_name(dev);

	the_controller = hsotg;
...
{
	struct s3c_hsotg *hsotg = the_controller;
	unsigned long flags;
	int rv;
	...
	if (hsotg->driver) {
		dev_err(hsotg->dev, "%s: device is busy\n", __func__);
		return -EBUSY;
	}
	if (!driver->bind || !driver->setup || driver->speed < USB_SPEED_FULL) {
		dev_err(hsotg->dev, "%s: invalid driver: bind %p setup %p "
			"speed %d\n", __func__, driver->bind, driver->setup,
			driver->speed);
		return -EINVAL;
	}
...
#if defined(MODULE)
	if (!driver->unbind) {
		dev_err(hsotg->dev, "%s: invalid driver: no unbind method\n",
			__func__);
		return -EINVAL;
	}
#endif
	...
	/* Bind the driver */
	if ((rv = device_add(&hsotg->gadget.dev)) != 0) {
		dev_err(hsotg->dev, "%s: error in device_add() : %d\n",
			__func__, rv);
		goto register_error;
	}

	if ((rv = driver->bind(&hsotg->gadget)) != 0) {
		dev_err(hsotg->dev, "%s: bind '%s' error %d\n", __func__,
			driver->driver.name, rv);
	...
	dwc2_udc_pullup(&hsotg->gadget, true);
...
{
	struct s3c_hsotg *udc = the_controller;
	...
	dev_info(udc->dev, "usb_gadget_unregister_driver() '%s'\n",
		driver->driver.name);

	s3c_hsotg_udc_stop(&udc->gadget, driver);
...
{
	return platform_driver_register(&dwc2_hsotg_driver);
}

static void __exit udc_exit(void)
{
	platform_driver_unregister(&dwc2_hsotg_driver);
}
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:dwc2-udc");
