...
struct flexcan_devtype_data {
	...
	u32 mb_cnt;		/* message buffers count */
};

#define FLEXCAN_MAX_IRQ_CNT	6
...
static struct flexcan_devtype_data fsl_kinetis_devtype_data = {
...
static inline int flexcan_has_and_handle_berr(const struct flexcan_priv *priv,
					      u32 reg_esr)
{
	return reg_esr & FLEXCAN_ESR_ERR_BUS;
}
...
static void flexcan_set_bittiming(struct net_device *dev)
{
	...
	dev_dbg(dev->dev.parent, "writing ctrl=0x%08x\n", reg);
...
static int flexcan_chip_start(struct net_device *dev)
{
	...
	for (i = FLEXCAN_TX_BUF_ID; i < priv->devtype_data->mb_cnt; i++) {
...
static struct platform_device_id flexcan_id_table[] = {
	...
	{ .name = "kinetis-flexcan",
		.driver_data = (kernel_ulong_t)&fsl_kinetis_devtype_data, },
	{ /* sentinel */ },
};
...
static int flexcan_probe(struct platform_device *pdev)
{
	...
	priv->can.clock.freq = priv->pdata->clock_frequency;
...
static int __init flexcan_module_init(void)
{
	printk(KERN_INFO "%s netdevice driver\n", DRV_NAME);
	return platform_driver_register(&flexcan_driver);
}

static void __exit flexcan_module_exit(void)
{
	platform_driver_unregister(&flexcan_driver);
	printk(KERN_INFO "%s: driver removed\n", DRV_NAME);
}

module_init(flexcan_module_init);
module_exit(flexcan_module_exit);
