...
smsc911x_irqhandler(int irq, void *dev_id)
{
	...
	if (unlikely(!(intsts & inten))) {
		printk(KERN_INFO "Spurious smsc911x irq.\n");
		return IRQ_HANDLED;
	}
...
static int smsc911x_drv_probe(struct platform_device *pdev)
{
	...
	/* Realview RevD active-high irq polarity:
	 * The polarity bits [4] and [1] must be set, *as well as*
	 * the irq enable bit [7]. If irqs are disabled, even though
	 * the polarity is changed, the overall polarity is wrong,
	 * and causes ongoing irqs. -> Not normal.
	 */
	smsc911x_reg_write(0x111, pdata, INT_CFG);
