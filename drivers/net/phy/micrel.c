...
#define MII_AFE1	0x11
#define KS8051_SLOW_OSC	(1 << 5)

int ks8051_suspend(struct phy_device *phydev)
{
	int regval, ret = 0;
	...
	if (phydev->dev_flags & MICREL_PHY_PM_SLOW_OSC) {
		...
		regval = phy_read(phydev, MII_AFE1);
		regval |= KS8051_SLOW_OSC;
		ret = phy_write(phydev, MII_AFE1, regval);
		msleep(5);
		if (ret < 0) goto out;
	}
...
int ks8051_resume(struct phy_device *phydev)
{
	int regval, ret = 0;
	mutex_lock(&phydev->lock);
	...
	regval = phy_read(phydev, MII_BMCR);
	regval &= ~BMCR_PDOWN;
	ret = phy_write(phydev, MII_BMCR, regval);
	if (ret) goto out;
...
int ks8051_probe(struct phy_device *phydev)
{
	return 0;
}
